...
var serverAddr = flag.String("server_addr", "127.0.0.1:10000", "The server address in the format host:port")

func GetPlayerId(name string) int {
  var res Player
  queryStringHead := "SELECT number FROM scrape_player where name="

  // Open database
  db, err := sql.Open("mysql", "root:mypass@tcp(127.0.0.1:3306)/baseball_py")
  if err != nil {
    panic(err.Error())
  }
  defer db.Close()
...
func GetPlayerName(name string) string {
  var res Player
  queryStringHead := "SELECT name FROM scrape_player where name LIKE "
